...
public class TableOrderModelAdmin extends AbstractTableModel {
    ...
    public TableOrderModelAdmin(Connection conn, String tableName) throws SQLException{
        super();
        TableOrderModelAdmin.conn = conn;
...
//цикл по всем записям таблицы
    ...
    public boolean updateDB(String tableName){
        ...
        for (int i = 0; i < contents.length; i++){
            ...
            sqlList.add("UPDATE `" + tableName + "` SET `дата`='" + objects[1] + "',`время`='"+ objects[2] +"',`пункт назн.`='" + objects[3] + "',`фио водителя`='" + objects[4] +"' WHERE `id`=" + objects[0]);
        }
    ...
    public boolean addDB(String tableName, String id, String date, String time, String destination, String name){

        ArrayList<String> sqlList = new ArrayList();
        sqlList.add("INSERT INTO `" + tableName +"`(`id`, `дата`, `время`, `пункт назн.`, `фио водителя`) VALUES ('" + id + "','" + date + "','"+ time +"','"+ destination +"','"+ name +"')");
